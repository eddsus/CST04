...
namespace DataManagement
{
    public class DataHandlerMain
    {
        ...
        public List<SharedDataTypes.Order> QueryOrders()
        {
            ...
            for (int i = 0; i < tempSharedOrders.Count; i++)
            {
                var tempOrderId = tempSharedOrders[i].OrderId;
                var temp2 = mainDb.OrderContents.Where(o => o.Order_ID.Equals(tempOrderId)).Select(oc => oc).ToList();
                tempSharedOrders[i].Content = converter.ConvertToSharedOrderContent(temp2);
            }
